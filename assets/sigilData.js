module.exports = {
  // General
  //? Meditation
  "Solidify Defense": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Meditation",
    "Sigil Value": 1,
    "HP Modification": "2/3/4/5/6",
    "Effect": "Gain an additional 2/3/4/5/6 **DEF** when adding **DEF** next time"
  },
  "HP": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Meditation",
    "Sigil Value": 1,
    "HP Modification": "2/3/4/5/6",
    "Effect": "HP + 2/3/4/5/6"
  },
  "Strong Attack": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Meditation",
    "Sigil Value": 1,
    "HP Modification": "1/2/3/4/5",
    "Effect": "Deal 2/3/4/5/6 more ATK in the next attack"
  },
  "Smash DEF": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Meditation",
    "Sigil Value": 1,
    "HP Modification": "3/3/5/5/5",
    "Effect": "Gain **Smash DEF** for the next 1/2/2/3/4 attacks"
  },
  "Carefree": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Meditation",
    "Sigil Value": 1,
    "HP Modification": 2,
    "Effect": "Deal 5/7/9/13/18 more ATK in the next **Normal Attack**"
  },
  //? Foundation
  "Regulate Breathing": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Foundation",
    "Sigil Value": 1,
    "HP Modification": "4/5/6/7",
    "Effect": "Consume 1 **Qi** at the end of this turn: HP and Max HP + 6/7/9/11"
  },
  "Expand Vitality": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Foundation",
    "Sigil Value": 1,
    "HP Modification": "4/6/8/10",
    "Effect": "Max HP + 14/16/18/20"
  },
  "Suppress": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Foundation",
    "Sigil Value": 1,
    "HP Modification": 2,
    "Effect": "Deal 1/2/3/4 **DMG** (increase 1 **DMG** for every 1 Cultivation higher than the opponent, maximum increase of 6/8/10/12)"
  },
  "Mechanism Assembly": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Foundation",
    "Sigil Value": 1,
    // "HP Modification": 0,
    "Effect": "**Mechanism Rune** + 3/4/5/6"
  },
  "Prevent": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Foundation",
    "Sigil Value": 2,
    "HP Modification": "6/7/8/9",
    "Effect": "First gain 7/10/13/16 **DEF** when receiving **ATK** or **DMG** next time"
  },
  "Absorb": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Foundation",
    "Sigil Value": 2,
    // "HP Modification": 6,
    "Effect": "Steal 6/8/10/12 HP from the opponent"
  },
  //? Virtuoso
  "Exorcise Evil": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 1,
    "HP Modification": 4,
    "Effect": "Repeat 2/3/4 times: randomly reduce 1 stack of **Debuff**"
  },
  "Surge of Divine Power": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 1,
    "HP Modification": "0/2/4",
    "Effect": "Gain an additional 1 stack when gaining **Increase ATK** next time"
  },
  "Dissipate Force": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 1,
    "HP Modification": "4/6/8",
    "Effect": "Remove 1 stack of the opponent's **Increase ATK** and **Guard Up**"
  },
  "Mechanism Shield": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 1,
    "HP Modification": "2/1/0",
    "Effect": "**Mechanism Rune** + 2/3/4\nGain 1 **DEF** for each stack of **Mechanism Rune**"
  },
  "Strengthen Fundamental Essence": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 2,
    "HP Modification": 6,
    "Effect": "HP + 5/7/9, **DEF** + 5/7/9, reduce 1 stack of **Internal Injury**, **Wound** and **Flaw**"
  },
  "Poison Tempering": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 2,
    "HP Modification": 2,
    "Effect": "Apply 2/3/4 stacks of **Internal Injury** after the next attack"
  },
  //? Immortality
  "Hexproof": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 1,
    "HP Modification": 4,
    "Effect": "Gain 2/3 **Hexproof**"
  },
  "Sigil Enhancement": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 1,
    "HP Modification": "6/8",
    "Passive Effect": "At the start of the battle, the **Sigil** in the next slot permanently increase 1 Phase (maximum up to Incarnation Phase)"
  },
  "Immovable": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 1,
    "HP Modification": "0/2",
    "Passive Effect": "At the end of each of your turns, if **Chase** was not triggered this turn, gain 3 **DEF**"
  },
  "Spirit Rune Mechanism": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 1,
    "HP Modification": 4,
    "Effect": "**Mechanism Rune** + 3/4\nConsume 1 **Qi** at the end of this turn: **Mechanism Rune** + 3"
  },
  "Speed": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 2,
    "HP Modification": 4,
    "Passive Effect": "**Speed** + 3/4"
  },
  "Bear": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 2,
    "HP Modification": "16/20",
    "Passive Effect": "You can add 1 more **Continuous** or **Consumption** card to your deck"
  },
  //? Incarnation
  "Disaster": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 1,
    // "HP Modification": 0,
    "Effect": "When the opponent plays a card that can be **Downgraded** next time, **Downgrade** it (restored after the battle)"
  },
  "Mechanism Engine": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 1,
    "HP Modification": 4,
    "Passive Effect": "Gain 1 **Mechanism Rune** at the start of each of your turns"
  },
  "Draw Dragon": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 2,
    "HP Modification": 10,
    "Effect": "When you play a card that can be **Upgraded** next time, **Upgrade** it (restored after the battle)"
  },
  "Immortal Trapping": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 2,
    "HP Modification": 8,
    "Effect": "Both players cannot trigger **Chase**, each lasts for 2 turns"
  },
  "Guard Up": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 2,
    "HP Modification": 4,
    "Effect": "Gain 2 **Guard Up**"
  },
  "Spirit Overflow": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 3,
    "HP Modification": 10,
    "Effect": "Gain an additional 5 **Qi** when increasing **Qi** the next time"
  },
  // Cloud Spirit Sword Sect
  //? Meditation
  "Cloud Rise": {
      "sect": "Cloud Spirit Sword Sect",
      "image": "https://i.imgur.com/AfFp7pu.png",
      "Starting Phase": "Meditation",
      "Sigil Value": 1,
      "HP Modification": "2/4/6/8/10",
      "Passive Effect": "Treated as having played **Cloud Sword** once at the start of the battle"
    },
  "Spirit Slash": {
      "sect": "Cloud Spirit Sword Sect",
      "image": "https://i.imgur.com/AfFp7pu.png",
      "Starting Phase": "Meditation",
      "Sigil Value": 1,
      "HP Modification": 2,
      "Passive Effect": "Deal 2/4/6/8/10 more ATK in the next **Spirit Sword** attack"
    },
  "Sword Practice": {
    "sect": "Cloud Spirit Sword Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Meditation",
    "Sigil Value": 1,
    "HP Modification": "2/4/6/8/6",
    "Effect": "Gain 1/1/1/1/2 **Sword Intent** after playing **Sword Slash** or **Sword Defence** next time"
  },
  //? Foundation
  "Clouds Gather and Stars Converge": {
    "sect": "Cloud Spirit Sword Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Foundation",
    "Sigil Value": 2,
    "HP Modification": "8/12/16/20",
    "Passive Effect": "After playing **Cloud Sword - Touch Sky** or **Cloud Sword - Pierce the Star**, all your cards are treated as **Cloud Sword** until the end of battle"
  },
  "Harm the Body": {
    "sect": "Cloud Spirit Sword Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Foundation",
    "Sigil Value": 1,
    "HP Modification": 4,
    "Passive Effect": "Reduce the opponent's HP by an additional 2/4/6/8 in the next attack, and it will definitely be treated as **Injured** the opponent"
  },
  "Mechanism Sword - Spirit": {
    "sect": "Cloud Spirit Sword Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Foundation",
    "Sigil Value": 1,
    // "HP Modification": 0,
    "Effect": "**Mechanism Rune** + 2/3/4/5\nConsume 1 **Mechanism Rune** in the next attack to deal 3 more ATK and gain **Ignore DEF** for that attack"
  },
  "Flying Fang": {
    "sect": "Cloud Spirit Sword Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Foundation",
    "Sigil Value": 2,
    "HP Modification": "4/5/6/8",
    "Effect": "After the next card attacks, refund the **Sword Intent** used for its attack (maximum 4/5/6/8 points)"
  },
  //? Virtuoso
  "Madness": {
    "sect": "Cloud Spirit Sword Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 1,
    "HP Modification": "4/6/8",
    "Effect": "The next card played is treated as **Unrestrained Sword**"
  },
  "Earth Fiend": {
    "sect": "Cloud Spirit Sword Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 1,
    "HP Modification": "2/3/4",
    "Effect": "Add **DEF** for each damage dealt to HP of your next ATK (maximum 8/9/10)"
  },
  "Mechanism Sword - Slash": {
    "sect": "Cloud Spirit Sword Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 1,
    "HP Modification": 2,
    "Effect": "**Mechanism Rune** + 3/4/5\nConsume all **Mechanism Rune** in the next attack, dealing 2 more ATK per stack"
  },
  "Necessity": {
    "sect": "Cloud Spirit Sword Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 2,
    "HP Modification": "10/14/18",
    "Passive Effect": "When gaining **Ignore DEF** via **Cloud Sword - Necessity**, also gain the same number of **Increase ATK**"
  },
  "Condense Sword Intent": {
    "sect": "Cloud Spirit Sword Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 2,
    "HP Modification": "10/5/0",
    "Effect": "Gain an additional 2/3/4 when increasing **Sword Intent** next time"
  },
  "Kunpeng": {
    "sect": "Cloud Spirit Sword Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 2,
    "HP Modification": "3/4/5",
    "Passive Effect": "**Giant Roc Spirit Sword** and **Giant Kun Spirit Sword** deal 5/6/7 more ATK and gain **Smash DEF**"
  },
  //? Immortality
  "Hundred Birds": {
    "sect": "Cloud Spirit Sword Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 1,
    "HP Modification": "0/2",
    "Passive Effect": "Reduce the **Qi** cost of **Spirit Sword** cards by 1"
  },
  "Concentrate on Formation": {
    "sect": "Cloud Spirit Sword Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 1,
    "HP Modification": "8/10",
    "Effect": "When playing a **Sword Formation** card next time, gain 2 **DEF** for every 1 **Sword Intent**"
  },
  "Mechanism Turtle": {
    "sect": "Cloud Spirit Sword Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 1,
    "HP Modification": 2,
    "Passive Effect": "**Mechanism Rune** + 2/3\nConsume 1 **Mechanism Rune** each time you increase **DEF** to gain an additional 3 **DEF**"
  },
  "Moon in Water": {
    "sect": "Cloud Spirit Sword Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 2,
    "HP Modification": 8,
    "Effect": "DEF + 6/8; gain the effect of **Moon Water Sword Formation** for 2/3 turns (DEF is not halved at the start of the turn)"
  },
  "Soft Heart": {
    "sect": "Cloud Spirit Sword Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 3,
    "HP Modification": "8/14",
    "Effect": "After playing **Cloud Sword - Softheart** next time, increase Max HP by 20/30 and trigger **Chase**"
  },
  "Flash Wind": {
    "sect": "Cloud Spirit Sword Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 3,
    "HP Modification": 14,
    "Passive Effect": "After playing **Cloud Sword - Flash Wind** each time, you can trigger **Chase** one more time this turn (can only take effect once per turn)"
  },
  //? Incarnation
  "Zero Form": {
    "sect": "Cloud Spirit Sword Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 1,
    "HP Modification": 4,
    "Passive Effect": "Every Time **Unrestrained Sword - Zero** is played, your Max HP increases by 20 and you gain 1 **Increase ATK**"
  },
  "Mechanism Sword - Shock": {
    "sect": "Cloud Spirit Sword Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 1,
    "HP Modification": 2,
    "Effect": "**Mechanism Rune** + 3\nFirst deal 2 **DMG** to the opponent in the next attack (repeat this **DMG** once for every 2 **Mechanism Rune** you have)"
  },
  "Spirit and Unrestrain": {
    "sect": "Cloud Spirit Sword Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 2,
    "HP Modification": 10,
    "Passive Effect": "Gain 1 **Qi** each time you play **Unrestrained Sword**; **Spirit Sword** played also counts as **Unrestrained Sword** played"
  },
  "Cloud Sea Wandering Dragon": {
    "sect": "Cloud Spirit Sword Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 3,
    "HP Modification": 12,
    "Passive Effect": "After playing the Card in the first slot of the deck, trigger **Chase** if you **Injured** the opponent\nAll your **Cloud Sword - Dragon Roam** gain **Chase**"
  },
  "Control Sword to Form Formation": {
    "sect": "Cloud Spirit Sword Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 3,
    // "HP Modification": 0,
    "Passive Effect": "Each time you gain **DEF**, gain an addition 2 **DEF** and deal 2 **DMG** to the opponent (can be triggered at most 15 times per battle)"
  },
  "Swing Sword to Split Heaven": {
    "sect": "Cloud Spirit Sword Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 3,
    "HP Modification": 12,
    "Effect": "Make an additional 24 ATK after playing a card with ATK next time"
  },
  // Heptastar Pavillion
  //? Meditation
  "Shift Stars": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Meditation",
    "Sigil Value": 1,
    "HP Modification": "1/3/5/7/9",
    "Effect": "Gain an additional grid as **Star Point** when playing **Shifting Stars** or **Starry Moon** next time"
  },
  "Hexagram Fall": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Meditation",
    "Sigil Value": 1,
    "HP Modification": "0/1/2/3/4",
    "Effect": "Deal 4/5/6/7/8 **DMG** to the opponent when adding **Hexagram** next time"
  },
  "Thunderbolt": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Meditation",
    "Sigil Value": 1,
    "HP Modification": "2/4/4/6/6",
    "Passive Effect": "For ATK with a random range, increase their ATK upper limit by 1/1/2/2/3"
  },
  //? Foundation
  "Gather Star Power": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Foundation",
    "Sigil Value": 1,
    "HP Modification": "2/4/6/8",
    "Effect": "Gain an additional 1 **Star Power** after playing **Dotted Around** or **Astral Fleche** next time"
  },
  "Dark Star Soul Injury": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Foundation",
    "Sigil Value": 1,
    "HP Modification": "2/4/6/8",
    "Passive Effect": "Effects that increase **Star Power** are changed to apply the same amount of **Internal Injury** to the opponent"
  },
  "Divine Hexagram": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Foundation",
    "Sigil Value": 1,
    "HP Modification": "2/4/6/8",
    "Effect": "Gain an additional 1 when adding **Hexagram** next time"
  },
  "Mechanism Star Instrument": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Foundation",
    "Sigil Value": 1,
    "HP Modification": "4/6/8/8",
    "Effect": "**Mechanism Rune** + 1/1/1/2\nFor the next 1 card not on **Star Point**, consume 2 **Mechanism Rune** to make that slot **Become Star Point**"
  },
  //? Virtuoso
  "Transform Star Spirit": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 1,
    "HP Modification": "6/8/10",
    "Passive Effect": "If **Qi** is insufficient when playing a card, consume 1 **Star Point** to gain 2 **Qi**"
  },
  "Thunder Break": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 1,
    "HP Modification": "2/4/6",
    "Effect": "When playing a **Thunder** card next time, apply 1 **Wound** on the opponent, and the ATK gains **Smash DEF**"
  },
  "Mechanism Cat": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 1,
    "HP Modification": 2,
    "Passive Effect": "**Mechanism Rune** + 2/3/4\nConsume 1 **Mechanism Rune** at the end of each of your turns to increase HP and Max HP by 3"
  },
  "Yin-Yang": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 2,
    "HP Modification": "4/8/12",
    "Passive Effect": "When gaining **Qi** by playing **Yin Yang Formation**, also gain the same number of **Hexagram**"
  },
  "Derive Star Points": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 2,
    "HP Modification": "7/11/15",
    "Passive Effect": "Slots 4 and 5 of the deck are treated as **Star Point**"
  },
  "Catch Cicada": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 3,
    "HP Modification": "2",
    "Passive Effect": "After playing a card with **Post Action**, increase the HP and Max HP by 2/3/4; When playing **Hunter Becomes Preyer**, gain +1 HP and Max HP for each point of damage it does to the opponent"
  },
  //? Immortality
  "Mechanism Rat": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 1,
    "HP Modification": 4,
    "Passive Effect": "**Mechanism Rune** + 3/4\nIf **Mechanism Rune** < 3 at the start of each of your turns, consume 1 **Qi** to increase **Mechanism Rune** by 3"
  },
  "Spring Return": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 2,
    "HP Modification": "2/6",
    "Effect": "When playing **Revitalized** or the card in the 8th slot of your deck next time, increase HP and Max HP by 3 for every 1 remaining **Qi**"
  },
  "Ruthless": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 1,
    "HP Modification": "2/4",
    "Effect": "**Ruthless Water** no longer consumes **Qi**, and deals 1 more ATK for every 1 stack of **Internal Injury** the opponent has"
  },
  "Trump Card": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 2,
    "HP Modification": "0/4",
    "Passive Effect": "The card in the 8th slot of your deck can directly trigger its **Post Action** effect"
  },
  "Shock Injury": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 2,
    "HP Modification": "10/14",
    "Passive Effect": "When **Repel Citta-Dharma** and **Hexagram Formacide** deal **DMG** each time, deal 1 more **DMG** for every 1 **Star Power** or **Increase ATK** you have"
  },
  "Sacrifice to Thunder": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 3,
    // "HP Modification": 0,
    "Effect": "When playing a **Thunder** card next time, reduce your HP by 2/4 and gain 4/5 **Hexagram**"
  },
  //? Incarnation
  "Blood Plum": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 1,
    "HP Modification": 10,
    "Passive Effect": "**Strike Twice** no long need **Qi**, but will cost 8 HP."
  },
  "Mechanism Snake": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 1,
    "HP Modification": 2,
    "Passive Effect": "**Mechanism Rune** + 3\nConsume 2 **Mechanism Rune** at the start of each of your turns to apply 1 **Internal Injury** to the opponent"
  },
  "Continue Heavenly Fate": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 2,
    "HP Modification": 5,
    "Passive Effect": "When your HP drops to <= 0 for the first time in battle, increase HP and Max HP by 3 for each **Hexagram** you have, then continue fighting if HP > 0"
  },
  "Heavenly Axis Reflects Stars": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 3,
    "HP Modification": 2,
    "Passive Effect": "All slots are treated as **Star Point**;Lose 1 Star Power and trigger **Chase** after playing **Polaris Citta-Dharma** for the first time"
  },
  "Ko Threat Extension": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 3,
    "HP Modification": 6,
    "Effect": "The next card on **Star Point** takes effect one more time when played"
  },
  "Aggravate Internal Injury": {
    "sect": "Heptastar Pavillion",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 3,
    "HP Modification": 6,
    "Effect": "Double the opponent's **Internal Injury**\nIf the opponent has no **Internal Injury**, instead apply 6 **Internal Injury**"
  },
  // Five Elements Alliance
  //? Meditation
  "Sharp Thrust": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Meditation",
    "Sigil Value": 1,
    "HP Modification": "1/2/3/4/5",
    "Effect": "Reduce the opponent's HP by an additional 3/4/5/6/7 when **Penetrate** is triggered next time"
  },
  "Hot Sand": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Meditation",
    "Sigil Value": 1,
    "HP Modification": "2/3/4/5/6",
    "Effect": "Deal 2/3/4/5/6 **DMG** to the opponent when playing **Earth Spirit** or **Fire Spirit** card next time"
  },
  "Nourish": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Meditation",
    "Sigil Value": 1,
    // "HP Modification": 0,
    "Effect": "Increase HP and Max HP by 4/6/8/10/12 when playing **Wood Spirit** or **Water Spirit** card next time"
  },
  //? Foundation
  "Five Elements Spirit Spin": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Foundation",
    "Sigil Value": 1,
    "HP Modification": "2/4/6/4",
    "Passive Effect": "Every time **Activate** is repeated on the activated **Five Elements**, deal 2/2/2/3 **DMG** to the opponent"
  },
  "Heart Pierce": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Foundation",
    "Sigil Value": 1,
    "HP Modification": "2/3/4/5",
    "Passive Effect": "After attacking with **Metal Spirit - Heart Pierce**, refund the **Penetrate** triggered by it (maximum 3/6/9/12)"
  },
  "Mechanism Spirit Seal": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Foundation",
    "Sigil Value": 1,
    "HP Modification": "4/6/8/8",
    "Effect": "**Mechanism Rune** + 1/1/1/2\nIn this turn, when playing a card whose **Five Elements** is not **Activated**, consume 2 **Mechanism Rune** to **Activate** its **Five Elements**"
  },
  "Young Seedling": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Foundation",
    "Sigil Value": 2,
    "HP Modification": "6/10/4/8",
    "Effect": "Gain 1/1/2/2 stacks of **Increase ATK** after playing the Foundation Phase **Wood Spirit** card next time"
  },
  //? Virtuoso
  "Earth's Firmness": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 1,
    "HP Modification": 2,
    "Effect": "Gain 6/8/10 **DEF** when playing an **Earth Spirit** card next time"
  },
  "Metal's Sharpness": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 1,
    "HP Modification": 4,
    "Effect": "Gain 2/3/4 **Penetrate** when playing a **Metal Spirit** card next time"
  },
  "Fire's Blaze": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 1,
    "HP Modification": 2,
    "Effect": "Reduce the opponent's HP and Max HP by 6/8/10 when playing a **Fire Spirit** card next time"
  },
  "Cosmos Mechanism": {
    "sect": "Five Elements Alliance",
    "image": "httpFive Elements Alliances://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 1,
    // "HP Modification": 0,
    "Effect": "**Mechanism Rune** + 1/2/3 (gain an additional 1 for each type of **Five Elements** in the deck)"
  },
  "Turbulence": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 2,
    "HP Modification": "4/8/12",
    "Effect": "Perform an additional attack when playing **Water Spirit - Turbulent** next time"
  },
  "Cosmos Subdue": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 2,
    "HP Modification": "12/16/20",
    "Effect": "When **Activating** any **Five Elements** next time, additionally **Activate** the **Five Elements** that correspond to the **Overcoming Interaction of Five Elements**"
  },
  //? Immortality
  "Water Spirit": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 1,
    "HP Modification": "2/0",
    "Effect": "Gain 2/3 **Qi** when playing a **Water Spirit** card next time"
  },
  "Blend Cosmos": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 1,
    "HP Modification": "0/2",
    "Passive Effect": "When counting the number of different **Five Elements** in your deck, it is treated as an additional +1"
  },
  "Lava": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 1,
    "HP Modification": "8/10",
    "Passive Effect": "Each time you play a **Fire Spirit** card, reduce your own **DEF** by up to 24/30, then reduce the opponent's HP and Max HP by the same amount"
  },
  "Mechanism Crocodile": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 1,
    "HP Modification": 6,
    "Passive Effect": "**Mechanism Rune** + 2/3\nConsume 2 **Mechanism Rune** when playing **Water Spirit** card to gain 1 **Force of Water**; consume 2 **Mechanism Rune** when playing **Earth Spirit** card to gain 4 **DEF**"
  },
  "Cliff": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 2,
    "HP Modification": "6/10",
    "Passive Effect": "Double the **DMG** deal to the opponent when the effect of **Earth Spirit - Cliff** is triggered"
  },
  "Fire Whip": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 2,
    "HP Modification": 4,
    "Passive Effect": "Make an additional 5/7 ATK x 2 after attacking with **Wood Spirit** or **Fire Spirit** card next time"
  },
  //? Incarnation
  "Refine Cauldron": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 1,
    "HP Modification": 9,
    "Effect": "Permanently turn the **Metal Spirit** card on the far right of your hand into a Level 1 **Metal Spirit** card of the next higher phase (maximum up to Incarnation Phase)"
  },
  "Mechanism Deer": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 1,
    "HP Modification": 4,
    "Passive Effect": "**Mechanism Rune** + 2\nConsume 2 **Mechanism Rune** when playing **Metal Spirit** card to gain 3 **Penetrate**; consume 2 **Mechanism Rune** when playing **Wood Spirit** card to gain 1 **Increase ATK**"
  },
  "Water Blade": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 2,
    "HP Modification": 8,
    "Passive Effect": "At the end of your turn, the **DMG** caused by **Force of Water** is changed to a single attack, and the HP reduced when **Injured** the opponent is increased by 50%"
  },
  "Build Fortress": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 2,
    "HP Modification": 8,
    "Effect": "Convert up to 24 **DEF** into equivalent HP and Max HP at the end of your turn, lasting for 2 turns"
  },
  "Spirit Formation Deployment": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 3,
    "HP Modification": 12,
    "Effect": "Trigger **Chase** after playing a **Spirit Formation** card next time"
  },
  "Heavenly Marrow": {
    "sect": "Five Elements Alliance",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 3,
    "HP Modification": 5,
    "Passive Effect": "The **Continuous** count of **Five Elements Heavenly Marrow Rhythm** is doubled, and it can also take effect when the deck contains only 2 types of **Five Elements**"
  },
  // Duan Xuan Sect
  //? Meditation
  "Resist Xuan": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Meditation",
    "Sigil Value": 1,
    "HP Modification": 2,
    "Effect": "Gain 3/5/7/9/12 **DEF** when receiving a **Debuff** next time"
  },
  "Fist Training": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Meditation",
    "Sigil Value": 1,
    "HP Modification": "2/3/4/5/6",
    "Effect": "Gain 2/3/4/5/7 **Physique** after playing **Exercise Fist** next time"
  },
  "Fist Wind": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Meditation",
    "Sigil Value": 1,
    "HP Modification": 2,
    "Effect": "Deal 2/4/6/8/10 **DMG** to the opponent when p[laying the next **HP Cost** card"
  },
  //? Foundation
  "Break Free": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Foundation",
    "Sigil Value": 1,
    "HP Modification": "4/6/8/10",
    "Effect": "At the end of this turn, reduce **Weakened**, **Flaw**, and **Entangled** by an additional 2 stacks each"
  },
  "Resurrection": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Foundation",
    "Sigil Value": 1,
    "HP Modification": "2/4/6/4",
    "Effect": "When playing HP cost cards for the next 1/1/1/2 times, gain HP equal to the amount of HP cost"
  },
  "Lure Enemy": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Foundation",
    "Sigil Value": 1,
    "HP Modification": "2/3/4/5",
    "Effect": "When you gain a **Debuff** next time, apply 2 stacks of **Flaw** on the opponent"
  },
  "Training Mechanism": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Foundation",
    "Sigil Value": 1,
    // "HP Modification": 0,
    "Effect": "**Physique** + 1\n**Mechanism Rune** + 2/3/4/5"
  },
  //? Virtuoso
  "Crash Fist Power": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 1,
    "HP Modification": "1/3/5",
    "Passive Effect": "Your **Crash Fist** deals 1 more ATK"
  },
  "Break Limit": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 1,
    "HP Modification": "4/6/8",
    "Effect": "The maximum DEF increase limit of **Detect-Horse Palms** is increased by 4/6/8, and the maximum ATK increase limit of **Windward Palms** is increased by 2/3/4"
  },
  "Suspend Momentum": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 1,
    "HP Modification": "3/5/7",
    "Effect": "Add 1 **Force** first during your next 2 attacks"
  },
  "Mechanism Tiger": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 1,
    "HP Modification": 2,
    "Passive Effect": "**Mechanism Rune** + 3/4/5\nConsume 1 **Mechanism Rune** each time you attack, dealing 2 more ATK for that attack"
  },
  "Solid Physique": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 2,
    "HP Modification": 4,
    "Effect": "**Physique** + 1/2/3; Gain 1 **DEF** for every 4 **Physique**"
  },
  "Nether Heart": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 2,
    "HP Modification": 9,
    "Effect": "Gain 1/2/3 **Internal Injury** and 1/2/3 **Regeneration**"
  },
  //? Immortality
  "Temper Nether Seal": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 1,
    "HP Modification": 3,
    "Effect": "Gain 1 **Physique** for every 2 stacks of **Debuff** you have (maximum gain 4/6)"
  },
  "Elusive": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 1,
    "HP Modification": "2/4",
    "Passive Effect": "The continuous effect of **Elusive Footwork** can also be triggered once per turn during the opponent's turn"
  },
  "Xuan Healing": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 1,
    "HP Modification": "4/6",
    "Effect": "Gain 1 HP for each stack of **Debuff** you have"
  },
  "Defensive Posture": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 1,
    "HP Modification": "4/6",
    "Passive Effect": "Gain 1 **DEF** each time you gain or lose 1 point of **Force**"
  },
  "Mechanism Lion": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 1,
    "HP Modification": 2,
    "Passive Effect": "**Mechanism Rune** + 2/3\nConsume 1 **Mechanism Rune** at the start of each of your turns to gain 1 **Force**"
  },
  "Inch Force": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 2,
    "HP Modification": "6/10",
    "Effect": "The attack of the next **Crash Fist** gains double **Force** bonus (cannot stack)"
  },
  //? Incarnation
  "Invoke Nether Spirit": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 1,
    "HP Modification": 2,
    "Effect": "**Qi** + 2, gain 2 **Internal Injury**"
  },
  "Mechanism Leopard": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 1,
    "HP Modification": 2,
    "Passive Effect": "**Mechanism Rune** + 3\nAfter playing a card, if no attack has been made this turn, consume 2 **Mechanism Rune** to gain 2 **Agility**"
  },
  "Blood Shadow Agility": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 2,
    "HP Modification": 4,
    "Effect": "Gain 1 **Agility** for every 1 HP in cost when playing the next **HP Cost** card"
  },
  "Nether Momentum": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 2,
    "HP Modification": 4,
    "Effect": "**Force** + 2 (gain an additional 2 if you have **Debuff**)"
  },
  "Imitate Heaven and Earth": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 3,
    "HP Modification": 12,
    "Passive Effect": "When **Physique** exceeds **Physique Cap** for the first time in battle, gain 2 **Increase ATK** and permanently increase **Physique Cap** by 3"
  },
  "Scarlet Blood Rage": {
    "sect": "Duan Xuan Sect",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 3,
    "HP Modification": 12,
    "Passive Effect": "Each time you play a HP cost card, deal **DMG** equal to the HP cost of the opponent"
  },
}


/*
  "": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Meditation",
    "Sigil Value": 0,
    "HP Modification": 0,
    "Effect": ""
  },
  "": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Foundation",
    "Sigil Value": 0,
    "HP Modification": 0,
    "Effect": ""
  },
  "": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Virtuoso",
    "Sigil Value": 0,
    "HP Modification": 0,
    "Effect": ""
  },
  "": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Immortality",
    "Sigil Value": 0,
    "HP Modification": 0,
    "Effect": ""
  },
  "": {
    "sect": "General",
    "image": "https://i.imgur.com/AfFp7pu.png",
    "Starting Phase": "Incarnation",
    "Sigil Value": 0,
    "HP Modification": 0,
    "Effect": ""
  },
*/