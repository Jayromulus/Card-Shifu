...
const sectIcons = {
  ...
  'Miscellaneous sigils': 'https://i.imgur.com/xE7FZ8w.png',
...
const sigils = require('../assets/sigilData')
const lookup = require('../assets/sigilLookup')
const sigilNames = Object.keys(lookup)
...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('sigil')
    .setDescription('View information on a provided sigil')
    .addStringOption(option =>
      option
        .setName('name')
        .setDescription('sigil name')
  ...
  async autocomplete(interaction) {
    ...
    //* uFuzzy testing
    let idxs = uf.filter(sigilNames, focusedValue);

    if (idxs != null && idxs.length > 0) {
      let infoThresh = 25

      if (idxs.length <= infoThresh) {
        let info = uf.info(idxs, sigilNames, focusedValue)
        let order = uf.sort(info, sigilNames, focusedValue)

        await interaction.respond(
          order.map(i => { return lookup[sigilNames[info.idx[i]]] })
        )
      } else {
        const newArr = [];
        ...
        for (let i = 0; i < limit; i++)
          newArr.push(lookup[sigilNames[idxs[i]]].name)
...
async function displayEmbed(interaction, flippedName) {
  ...
  if (flippedName) {
    selected = sigils[flippedName.value]
  } else if (interaction.options.getString('name') === 'random') {
    const randIndex = Math.floor(Math.random() * (sigilNames.length - 1 + 1))
    
    selectedName = sigilNames[randIndex]
    selected = sigils[selectedName]
  } else {
    selectedName = interaction.options.getString('name')
    selected = sigils[selectedName]
  }
  // const level = parseInt(interaction.options.getNumber('level')) //! potential future sigil level integration
  
  // console.log({selected});
  const keys = Object.keys(selected)

  const fields = await keys.map((field) => ({ name: field, value: `${selected[field]}`, inline: field === "Sigil Value" || field === "HP Modification" }))
  fields.shift()
  const iconFromSigil = fields.shift()
  const icon = (isPenguin && selectedName === 'Envelop in Disaster') ? penguin : iconFromSigil.value !== 'https://i.imgur.com/AfFp7pu.png' ? iconFromSigil.value : sectIcons[selected.sect]
  isPenguin = false;
  // console.warn({icon})
  ...
  const sigilEmbed = new EmbedBuilder()
    ...
    // .setImage(selected.image) //! future sigil image integration
    // .setFooter({ text: selected.sect })
    .setFooter({ text: selected.sect, iconURL: sectIcons[selected.sect] })

  // await interaction.reply({ embeds: [sigilEmbed], ephemeral: true }) //flags: MessageFlags.Ephemeral
  await interaction.reply({ embeds: [sigilEmbed] }) //flags: MessageFlags.Ephemeral
  // const response = await interaction.reply(!fortuneTeller ? { embeds: [sigilEmbed] } : { embeds: [sigilEmbed], components: [row], withResponse: true }) //flags: MessageFlags.Ephemeral
  ...
  //     await confirmation.update({ embeds: !fortuneTeller ? { embeds: [sigilEmbed] } : { embeds: [sigilEmbed], components: [row], withResponse: true } });
